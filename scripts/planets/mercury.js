...
function startMercuryAnimation() {
    ...
    function createMercury()
    {
        ...
        const colDetail = new THREE.Color('#e0e0e0');
        ...
        // 优化1：超轻量分帧（每帧仅2500个，和金星一致）
        let generatedCount = 0;
        const BATCH_SIZE_PER_FRAME = 2500;

        function generateParticleBatch() {
            const endIdx = Math.min(generatedCount + BATCH_SIZE_PER_FRAME, particleCount);
            
            for (let i = generatedCount; i < endIdx; i++) {
                let r = 5.0;
            ...
            if (generatedCount < particleCount) {
                // 优化2：让出主线程，确保光标/UI优先（和金星一致）
                setTimeout(() => requestAnimationFrame(generateParticleBatch), 0);
            }
        }
        ...
        function addMercuryParticles()
        {
            ...
            const endIdx = Math.min(currentIndex + batchSize, generatedCount, positions.length / 3);
            
            if (endIdx > currentIndex) {
                const posArray = positions.slice(0, endIdx * 3);
                const colArray = colors.slice(0, endIdx * 3);
